...
UltimaSituacao AS (
    SELECT 
        ...
        CODIGO_SITUACAO, 
        DATA_ALTERACAO,
        ROW_NUMBER() OVER(PARTITION BY CODIGO_GRUPO, CODIGO_COTA, VERSAO ORDER BY DATA_ALTERACAO DESC, VERSAO_HIST DESC) as rn
    FROM COTAS_SITUACOES
)
...
SELECT 
    ...
    ct.CODIGO_COTA as Cota,
    ct.VERSAO as [Versão],
    cts.VERSAO_HIST as [Versão Historico],
    cts.CODIGO_SITUACAO as [Situação],
    sc.NOMENCLATURA as [Descrição da Situação],
    FORMAT(cts.DATA_ALTERACAO,'dd/MM/yyyy','en-US') as [Data da Situação],
    FORMAT(ct.DATA_ADESAO,'dd/MM/yyyy','en-US') as 'Adesão',
    ...
    fp.[4ª Parc. Vencimento], fp.[4ª Parc. Pagamento],
...
LEFT JOIN UltimaSituacao cts 
    ...
    AND ct.VERSAO = cts.VERSAO
    AND cts.rn = 1
LEFT JOIN SITUACOES_COBRANCAS sc
    ON cts.CODIGO_SITUACAO = sc.CODIGO_SITUACAO
LEFT JOIN REPRESENTANTES rep
    ON ct.CODIGO_REPRESENTANTE = rep.CODIGO_REPRESENTANTE
LEFT JOIN FinanceiroPivot fp
